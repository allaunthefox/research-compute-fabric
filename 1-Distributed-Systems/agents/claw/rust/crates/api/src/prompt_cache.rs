...
// Aggressive caching: compressed context produces stable hashes,
// so cached responses stay valid much longer than with raw content.
// SUBSTRATE_COMPLETION_TTL_SECS env var overrides at runtime.
const DEFAULT_COMPLETION_TTL_SECS: u64 = 4 * 3600; // 4 hours
const DEFAULT_PROMPT_TTL_SECS: u64 = 2 * 3600; // 2 hours
...
impl PromptCacheConfig {
    ...
    pub fn new(session_id: impl Into<String>) -> Self {
        let completion_ttl = std::env::var("SUBSTRATE_COMPLETION_TTL_SECS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(DEFAULT_COMPLETION_TTL_SECS);
        let prompt_ttl = std::env::var("SUBSTRATE_PROMPT_TTL_SECS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(DEFAULT_PROMPT_TTL_SECS);
        Self {
            session_id: session_id.into(),
            completion_ttl: Duration::from_secs(completion_ttl),
            prompt_ttl: Duration::from_secs(prompt_ttl),
...
impl PromptCache {
    ...
    pub fn lookup_completion(&self, request: &MessageRequest) -> Option<MessageResponse> {
        ...
        let Some(entry) = entry else {
            // File cache miss — check substrate cache before giving up
            if let Some(substrate_response) =
                substrate_cache_lookup(&request_hash, DEFAULT_PROMPT_TTL_SECS)
            {
                ...
                // Promote to file cache for faster future lookups
                write_completion_entry(&paths, &request_hash, &substrate_response);
                persist_state(&inner);
                return Some(substrate_response);
            }
            let mut inner = self.lock();
        ...
        inner.stats.last_completion_cache_key = Some(request_hash.clone());
        if expired {
            // File cache expired — check substrate cache (longer-lived)
            if let Some(substrate_response) =
                substrate_cache_lookup(&request_hash, DEFAULT_PROMPT_TTL_SECS)
            {
                inner.stats.completion_cache_hits += 1;
    ...
    fn record_usage_internal(
        ...
        if let Some(response) = response {
            ...
            // Also persist to substrate cache for cross-session reuse
            substrate_cache_response(&request_hash, response);
...
// ── Substrate response cache ────────────────────────────────────────────────
//
// Persists cached responses as JSON files in a substrate-aligned directory
// so they survive across process restarts and sessions.  The Python
// context gate can also read/write this directory to share cache state.

/// Directory for substrate-backed response cache.
/// Returns `None` if substrate caching is disabled (e.g. during tests).
fn substrate_response_cache_dir() -> Option<PathBuf> {
    // Disable substrate cache during tests or when explicitly opted out
    if std::env::var_os("SUBSTRATE_CACHE_DISABLE").is_some() {
        return None;
    }
    if let Some(home) = std::env::var_os("HOME") {
        let research_stack = PathBuf::from(&home).join("Research Stack");
        if research_stack.is_dir() {
            return Some(research_stack.join("out").join("response_cache"));
        }
    }
    Some(base_cache_root().join("substrate-responses"))
}

/// Write a response to the substrate cache for cross-session persistence.
pub fn substrate_cache_response(request_hash: &str, response: &MessageResponse) {
    let Some(dir) = substrate_response_cache_dir() else {
        return;
    };
    ...
    let _ = write_json(&dir.join(format!("{request_hash}.json")), &entry);
}

/// Look up a response from the substrate cache.
pub fn substrate_cache_lookup(
    request_hash: &str,
    max_age_secs: u64,
) -> Option<MessageResponse> {
    let dir = substrate_response_cache_dir()?;
    let path = dir.join(format!("{request_hash}.json"));
    let entry = read_json::<CompletionCacheEntry>(&path)?;
    if entry.fingerprint_version != current_fingerprint_version() {
...
mod tests {
    ...
    fn test_env_lock() -> std::sync::MutexGuard<'static, ()> {
        ...
        // Disable substrate cache in tests to avoid cross-contamination
        std::env::set_var("SUBSTRATE_CACHE_DISABLE", "1");
