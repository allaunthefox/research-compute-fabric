...
use crate::context_compress::ContextCompressor;
            ...
            for (tool_use_id, tool_name, input) in pending_tool_uses {
                ...
                let result_message = match permission_outcome {
                    PermissionOutcome::Allow => {
                        ...
                        // Context compression: run large tool outputs through
                        // the local substrate cache / hyperlut before they
                        // burn context tokens. Offensively boring by design.
                        if !is_error {
                            if let Some(compressed) =
                                self.context_compressor.maybe_compress(&tool_name, &output)
                            {
